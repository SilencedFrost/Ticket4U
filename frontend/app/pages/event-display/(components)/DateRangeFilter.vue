<template>
    <div class="position-relative">
        <button class="btn btn-filter-primary d-flex align-items-center gap-2" :class="isMobile ? 'btn-filter-mobile' : 'btn-filter-desktop'" @click="$emit('toggle')">
        ...
        <div v-if="show" class="dropdown-popup position-absolute bg-reactive-secondary border border-1 rounded-3 shadow" style="min-width: 600px">
            <div class="p-4">
                <!-- Preset Options -->
                <div class="preset-options d-flex flex-wrap gap-2 pb-3 mb-3 border-bottom border-secondary">
                    <button v-for="preset in presets" :key="preset.value" :class="[
