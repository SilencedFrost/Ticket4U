...
function startResize(e: MouseEvent) {
  ...
  function onMove(ev: MouseEvent) {
    cartWidth.value = Math.min(700, Math.max(300, startWidth + startX - ev.clientX));
  }
  function onUp() {
...
<template>
  <div
    class="event-payment-wrapper container-fluid bg-reactive-primary text-reactive-primary vh-100 overflow-hidden p-0"
  >
    ...
    <div v-else-if="event" class="h-100">
      <div class="d-none d-lg-flex h-100">
        ...
        <div
          class="cart-sidebar bg-reactive-primary d-flex flex-column h-100"
      ...
      <div class="d-flex d-lg-none flex-column h-100 position-relative">
        ...
        <transition name="fade">
          <div
            v-if="drawerOpen"
            style="position: fixed; inset: 0; background: rgba(0, 0, 0, 0.4); z-index: 199"
          ...
          >
            <div class="d-flex align-items-center gap-2">
              ...
              <span class="fw-semibold text-reactive-primary">{{
                $t('select_ticket.cart.title')
              }}</span>
              <span v-if="totalTickets > 0" class="badge bg-primary rounded-pill">{{
                totalTickets
              }}</span>
            </div>
            <div class="d-flex align-items-center gap-3">
              <span v-if="totalPrice > 0" class="text-primary fw-bold">{{
                formatPrice(totalPrice)
              }}</span>
              <i
                class="bi fs-5 text-reactive-secondary"
                :class="drawerOpen ? 'bi-chevron-down' : 'bi-chevron-up'"
              />
            </div>
          </div>
          <div
