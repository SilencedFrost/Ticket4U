  ...
  <div class="rich-editor" :class="{ 'is-focused': isFocused, 'is-disabled': disabled }">
    ...
    <div class="rich-editor-toolbar d-flex flex-wrap align-items-center gap-1 p-2">
      ...
      <div class="btn-group btn-group-sm me-1">
        ...
        <button type="button" class="toolbar-btn" :class="{ active: editor?.isActive('orderedList') }" title="Numbered list" @click="editor?.chain().focus().toggleOrderedList().run()">
      ...
      <div class="d-flex align-items-center gap-1 me-1">
        ...
        <button v-if="editor?.isActive('textStyle') || editor?.isActive('highlight')" type="button" class="toolbar-btn" title="Clear formatting" @click="clearFormatting">
    ...
    <div v-if="showLinkModal" class="modal-backdrop-simple" @click.self="showLinkModal = false">
      <div class="link-modal bg-reactive-secondary p-3 rounded-3 shadow-lg" style="min-width:320px;">
        <div class="fw-semibold text-reactive-primary mb-2"><i class="bi bi-link-45deg me-2"/>Insert Link</div>
        <input
          v-model="linkUrl"
          type="url"
...
// Sync external changes (e.g. when loading event data)
...
<style>
/* Global — TipTap editor content styles */
.rich-editor-inner {
  outline: none;
...
.rich-editor-inner ul, .rich-editor-inner ol { padding-left: 1.4em; margin-bottom: 0.6em; }
