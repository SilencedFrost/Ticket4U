...
const formatEventPrice = (price: number): string => {
  if (price === 0) return $t('event_display.label.free');
  return price
    ? `${$t('common.price.from')} ${formatPrice(price)}`
  ...
  <div>
    <div class="row g-3 g-md-4">
      <div v-for="event in events" :key="event.id" class="col-6 col-md-4 col-lg-3">
        <div
          class="event-card-wrapper cursor-pointer"
          role="button"
          tabindex="0"
          @click="$emit('event-click', event.id)"
          @keypress.enter="$emit('event-click', event.id)"
        >
          <!-- Event Image -->
