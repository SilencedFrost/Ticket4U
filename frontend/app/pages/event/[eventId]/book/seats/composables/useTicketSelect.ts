...
function buildLayoutZone(z: RawLayoutZone, zoneById: Map<string, Zone>): LayoutZone {
  ...
  const zoneType: 'sitting' | 'standing' =
    zoneData != null ? (zoneData.isStanding ? 'standing' : 'sitting') : (z.zone_type ?? 'standing');
...
function buildFloors(layoutJson: string | null, zones: Zone[]): Floor[] {
  ...
  let parsed: {
    venueMode?: boolean;
    floors?: RawLayoutFloor[];
    stage?: RawLayoutFloor['stage'];
    zones?: RawLayoutZone[];
  };
  try {
    parsed = JSON.parse(layoutJson) as typeof parsed;
  } catch {
    return [];
  }
  ...
  if (parsed.floors && parsed.floors.length > 0) {
    ...
    rawFloors = [
      { floor_order: 1, floor_name: 'Main Floor', stage: parsed.stage, zones: parsed.zones },
    ];
  } else {
    return [];
  }
...
function mapSeatingPlan(data: SeatingPlan): { tickets: Ticket[]; floors: Floor[] } {
  const tickets: Ticket[] = data.zones.map(
    (zone, i): Ticket => ({
      ...zone,
  ...
  async function fetchTicketSelect(eventId: string, sessionId: string) {
    ...
    try {
      ...
      event.value = {
        ...
        date: start.toLocaleDateString(locale.value, {
          day: 'numeric',
          month: 'long',
          year: 'numeric',
        }),
