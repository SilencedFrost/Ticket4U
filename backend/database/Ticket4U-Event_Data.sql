...
INSERT INTO public.events (
	...
	start_date, end_date, status, banner_url, description,
...
INSERT INTO public.zones (id, event_id, name, is_standing, capacity, quantity_sold, price, description, perks)
SELECT uuidv7(), id, 'VVIP', false, 50, 0, 2500000, 
	'Khu vực VIP cao cấp với dịch vụ đặc biệt', 
	'["Vòng tay check-in VIP", "Goodie bag", "Meet & Greet"]'::jsonb
...
SELECT uuidv7(), id, 'GA Standing', true, 800, 0, 650000, 
	...
	'["Vòng tay check-in", "Nước suối"]'::jsonb
FROM public.events WHERE name = 'Hà Anh Tuấn: Chân Trời Rực Rỡ';
...
SELECT uuidv7(), id, 'Cư Dân VIP', false, 200, 0, 1500000, 
	...
	'["Bộ quà tặng Cư Dân", "Lối đi ưu tiên", "Nước uống miễn phí"]'::jsonb
...
SELECT uuidv7(), id, 'Khu VVIP (Gần ca sĩ)', false, 50, 0, 1800000, 
	...
	'["Một phần nước tự chọn", "Đĩa CD nhạc Trịnh"]'::jsonb
...
SELECT uuidv7(), id, 'SVIP Deck', false, 100, 0, 4500000, 
	...
	'["Private Bar", "Quà tặng từ nhà tài trợ", "Lối đi VIP"]'::jsonb
...
SELECT uuidv7(), id, 'VIP Premium', false, 50, 0, 800000, 
	...
	'["Tài liệu VIP", "Suất ăn trưa cao cấp", "Networking session"]'::jsonb
...
SELECT uuidv7(), id, 'Standard', false, 200, 0, 400000, 
	...
	'["Tài liệu hội thảo", "Teabreak giữa giờ"]'::jsonb
FROM public.events WHERE name = 'Vietnam Tech Summit 2025';
...
SELECT uuidv7(), id, 'Courtside VIP', false, 50, 0, 2500000, 
	...
	'["Áo đấu phiên bản giới hạn", "F&B phục vụ tại chỗ"]'::jsonb
...
SELECT uuidv7(), id, 'Premium (Kèm tài liệu)', false, 50, 0, 450000, 
	...
	'["Tài liệu in màu", "Chứng chỉ", "Networking lunch"]'::jsonb
...
SELECT uuidv7(), id, 'VIP (Kèm quà tặng)', false, 200, 0, 1200000, 
	...
	'["Sổ tay Van Gogh", "Ưu tiên chụp ảnh tại khu vực đặc biệt"]'::jsonb
